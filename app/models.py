...
class User(UserMixin, db.Model):
    ...
    username: sqlalchemy.orm.Mapped[str] = sqlalchemy.orm.mapped_column(
        sqlalchemy.String(64), index=True, unique=True
    )

    email: sqlalchemy.orm.Mapped[str] = sqlalchemy.orm.mapped_column(
        sqlalchemy.String(120), index=True, unique=True
    )

    password_hash: sqlalchemy.orm.Mapped[Optional[str]] = sqlalchemy.orm.mapped_column(
    ...
    about_me: sqlalchemy.orm.Mapped[Optional[str]] = sqlalchemy.orm.mapped_column(
...
class Post(db.Model):
    ...
    user_id: sqlalchemy.orm.Mapped[int] = sqlalchemy.orm.mapped_column(
        sqlalchemy.ForeignKey(User.id), index=True
    )

    author: sqlalchemy.orm.Mapped[User] = sqlalchemy.orm.relationship(
